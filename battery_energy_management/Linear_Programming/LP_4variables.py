...
'''
class BatteryContoller_old(object):# 1st Place
    step = 960      # 10 дней моделирования

    def propose_state_of_charge(self,   # метод для объекта класса BatteryController, который применяется сам к себе [суть: считает предполагаемое состояние заряда батареи на след. шаг]
                                site_id,
                                timestamp,
                                battery,
        ...
        if (self.step > 1): number_step = min(96, self.step)    # число шагов либо 96, либо меньше (если мало осталось) {сделано для универсальности программы}
        #
        price_buy = price_buy.tolist()                          # переводим в лист прогнозы
        price_sell = price_sell.tolist()
        load_forecast = load_forecast.tolist()
        pv_forecast = pv_forecast.tolist()
        #
        energy = [None] * number_step                           # лист энергии

        for i in range(number_step):                            # заполняем лист энергии (если pv выработало энергию больше 50В, то считаем разность, иначе считаем что она вообще не работала)
            if (pv_forecast[i] >= 50):
                energy[i] = load_forecast[i] - pv_forecast[i]
            else:
                energy[i] = load_forecast[i]
        # battery
        capacity = battery.capacity                             # подгружаем себе в переменные (для удобства) данные батареи
        charging_efficiency = battery.charging_efficiency
        discharging_efficiency = 1. / battery.discharging_efficiency    # эффективность разряда (число большее 1, т.е. обозначающее то, сколько у нас будет имется до разряда)
        current = capacity * battery.current_charge             # так как current_charge это доля (от 0 до 1), то энергия будет определяться произведением вместимости на долю
        limit = battery.charging_power_limit
        dis_limit = battery.discharging_power_limit
        limit /= 4.                                             # обозначаем предел заряда батареи (считаем что за раз лишь 1/4 пожем зарядить)
        dis_limit /= 4.                                         # аналогично для разряда


        #solver = pywraplp.Solver.CreateSolver('GLOP')           # создаем "объект солвер" и будем туда запихивать ограничения и целевую
        solver = pywraplp.Solver.CreateSolver(f'{method_name}')

        # Variables: all are continuous
        charge = [solver.NumVar(0.0, limit, "c" + str(i)) for i in range(number_step)]              # создаем переменные (запихнем их в лист), с пределами [0;limit] и именем "ci" в кол-ве 96 штук или меньше
        dis_charge = [solver.NumVar(dis_limit, 0.0, "d" + str(i)) for i in range(number_step)]
        battery_power = [solver.NumVar(0.0, capacity, "b" + str(i)) for i in range(number_step + 1)]    # на одну больше, чтобы после вычисления туда запихнуть последний результат (то что предскажем)
        grid = [solver.NumVar(0.0, solver.infinity(), "g" + str(i)) for i in range(number_step)]

        # Objective function
        objective = solver.Objective()
        for i in range(number_step):                                            # целевая - сумма от i=0:95 [здесь SetCoefficient(a,b) это objective += a*b]
            objective.SetCoefficient(grid[i], price_buy[i])                     # (энергия что нужна сети [i], цена покупки единицы энергии [i])
            objective.SetCoefficient(grid[i], price_buy[i] - price_sell[i])     # слагаемое [штраф], которое позволяет отслеживать: не покупаем за слишком большую цену энергию и покупаем за малую
            objective.SetCoefficient(charge[i], price_sell[i] + price_buy[i] / 1000.)   #
            objective.SetCoefficient(dis_charge[i], price_sell[i])              # (энергия c батареи что отдали [i], цена продажи единицы энергии [i])
        objective.SetMinimization()                                             # задаем, что целевую функцию будем минимизировать

        # 3 Constraints
        c_grid = [None] * number_step           # (переменные для условия - количество покупаемой энергии на шаге [i])
        c_power = [None] * (number_step + 1)    # (переменные для условия баланса - энергия батарейки шаге [i])

        # 1st constraint
        c_power[0] = solver.Constraint(current, current)    # условие "текущее кол-во энергии батарейки == current":  current <= (c_power[0]) <= current
        c_power[0].SetCoefficient(battery_power[0], 1)      #                                                         current <= (battery_power[0]) <= current

        # 2nd and 3rd constraint
        for i in range(0, number_step):                                         # для 96 шагов вперед мы записываем условия на переменные
            # second constraint
            c_grid[i] = solver.Constraint(energy[i], solver.infinity())
            c_grid[i].SetCoefficient(grid[i], 1)                                # grid[i] - кол-во энергии, необходимое сети, т.е. что нужно купить на шаге [i]
            c_grid[i].SetCoefficient(charge[i], -1)                             # charge[i] - кол-во энергии, которым заряжаем батарею на шаге [i]
            c_grid[i].SetCoefficient(dis_charge[i], -1)                         # dis_charge[i] - кол-во энергии, которое мы изъяли из батареи на шаге [i]
                                                                                # energy[i] <= grid[i] - charge[i] - dis_charge[i] <= inf    {здесь dis_charge <= 0, т.е. затраты со знаком +, а прибыль с -}
            # third constraint
            c_power[i + 1] = solver.Constraint(0, 0)        # от 1 до 96 (в пред от 0 до 95) у нас c_power[i+1] == 0
            ...
            c_power[i + 1].SetCoefficient(battery_power[i + 1], -1)     # 0 <= charge[i] * charge_eff + dis_charge[i] * discharge_eff + battery_power[i] - battery_power[i+1] <= 0   [условие баланса]

        # solve the model
        solver.Solve()

        if forecasts_correction == True:
         #Если батарея 2 раза последовательно разряжалась, то 3ий раз нужно зарядить
            ...
            if ((energy[0] > 0) & (charge[0].solution_value() <= 0)):
                ...
                last = energy[0]
                for n in range(1, number_step):
                    if ((energy[n] < 0) | (charge[n].solution_value() > 0) | (price_sell[n] != price_sell[n - 1]) | (
...
class BatteryController(object):
    ...
    def propose_state_of_charge(self,
        ...
        grid = [model.addVar(name="g" + str(i), lb=0.0, vtype=grb.GRB.CONTINUOUS) for i in range(number_step)]
        ...
        for i in range(0, number_step):
            model.addConstr(grid[i] - charge[i] - dis_charge[i] >= energy[i], f'constraint_2_{i}')
        ...
        model.setObjective(grb.quicksum([grid[i] * price_buy[i] + grid[i] * (price_buy[i] - price_sell[i]) + charge[i] * (price_sell[i] + price_buy[i] / 1000.) + dis_charge[i] * price_sell[i] for i in range(number_step)]), grb.GRB.MINIMIZE)
        ...
        '''
        battery_power[0] = battery_power[0].X    # перевожу gurobi value в float
        for i in range(0, number_step):
            charge[i] = charge[i].X
            dis_charge[i] = dis_charge[i].X
            battery_power[i+1] = battery_power[i+1].X
            grid[i] = grid[i].X
        '''
...
if __name__ == '__main__':
    ...
    if forecasts_correction == False:
        results_df.to_csv(f'results_GUROBI_(no_correction).csv')
        my_average_function(f'results_GUROBI_(no_correction)')
    else:
        results_df.to_csv(f'results_GUROBI_(with_correction).csv')
        my_average_function(f'results_GUROBI_(with_correction)')
